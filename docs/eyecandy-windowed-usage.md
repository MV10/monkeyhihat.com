...
Refer to the `demo` project's `Program.cs` for a simple example of how to wire up Serilog logging. As of v3.2.0 eyecandy is tied into the OpenGL debug-message error signaling facility. You'll get detailed error messages and a stack trace -- a huge improvement over the old `GetError` approach.
